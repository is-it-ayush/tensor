 ...
 * Scales two vectors `a` and `b` and stores the result in `r`.
 */
T_SCOPE void vec3_scale(vec3 a, vec3 b, vec3 r);
/**
 * Calculates the dot product of two vectors `a` and `b`.
 */
 ...
 * Scales two vectors `a` and `b` and stores the result in `r`.
 */
T_SCOPE void vec4_scale(vec4 a, vec4 b, vec4 r);
/**
 * Calculates the dot product of two vectors `a` and `b`.
 */
...
/**
 * Generates a 4x4 rotation matrix around an axis `axis` by an angle `angle` and stores the result in `r`.
 */
T_SCOPE void m4x4_make_rotation(vec3 axis, float angle, m4x4 r);
...
#define TENSOR_IMPLEMENTATION // comment this line to include the implementation
...
T_SCOPE void print_vec2(vec2 v) { printf("[%f, %f]\n", v[0], v[1]); }
T_SCOPE void print_vec3(vec3 v) {
  printf("[%f, %f, %f]\n", v[0], v[1], v[2]);
}
T_SCOPE void print_vec4(vec4 v) {
...
T_SCOPE void vec4_rotate(vec4 a, float angle, vec3 axis, vec4 r) {
  m4x4 m_rot;
  m4x4_make_rotation(axis, angle, m_rot);
...
T_SCOPE void m4x4_make_rotation(vec3 axis, float angle, m4x4 r) {
  ...
  float t_cos, t_sin, mag, t_minus_cos;
  float a_rad = deg_to_rad(angle);

  t_cos = cosf(a_rad);
  t_sin = sinf(a_rad);
  t_minus_cos = 1.0f - t_cos;

  mag = 1.0f / vec3_mag(axis);
  vec3_scale(axis, (vec3){mag, mag, mag}, axis_n); // normalize axis
  vec3_scale(axis_n, (vec3){t_minus_cos, t_minus_cos, t_minus_cos}, v_cos);
  vec3_scale(axis_n, (vec3){t_sin, t_sin, t_sin}, v_sin);
  vec3_scale(axis_n, (vec3){v_cos[0], v_cos[0], v_cos[0]}, r[0]);
  vec3_scale(axis_n, (vec3){v_cos[1], v_cos[1], v_cos[1]}, r[1]);
  vec3_scale(axis_n, (vec3){v_cos[2], v_cos[2], v_cos[2]}, r[2]);
...
T_SCOPE double deg_to_rad(double deg) { return deg * (MATH_PI / 180.0); }
T_SCOPE double rad_to_deg(double rad) { return rad * (180.0 / MATH_PI); }
